...
export default function RolePanelPage() {
  ...
  const currentRole = panel?.user.role ?? role ?? 'sekolah'
  const isSchool = currentRole === 'sekolah'
  const isSppg = currentRole === 'sppg'
  ...
  useEffect(() => {
    ...
    const expectedPath = panel.user.role === 'sppg'
      ? '/panel/sppg'
      : panel.user.role === 'sekolah'
        ? '/panel/sekolah'
        : '/panel'
  ...
  const handleReportSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = isSppg ? '/panel/distribution' : '/panel/report'
      let response: { message: string }

      if (isSppg) {
        response = await apiRequest<{ message: string }>(endpoint, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="role-panel-shell">
        ...
        <section className="role-panel-grid">
          ...
          <div className="role-panel-card">
            <header className="role-panel-card-head">
              <div>
                <h2>{isSppg ? 'Input Laporan Distribusi Porsi' : 'Input Laporan Sekolah'}</h2>
                <p>{isSppg ? 'Isi distribusi harian ke sekolah yang dilayani.' : 'Isi laporan harian sekolah.'}</p>
              </div>
            </header>
            ...
            <form className="role-panel-form" onSubmit={handleReportSubmit}>
              ...
              <button type="submit" className="role-panel-button" disabled={savingReport}>
                {savingReport ? 'Menyimpan...' : isSppg ? 'Simpan Distribusi' : 'Simpan Laporan'}
              </button>
            </form>
          </div>
        </section>
        ...
        <section className="role-panel-card role-panel-full">
          <header className="role-panel-card-head">
            <div>
              ...
              <p>{isSppg ? 'Distribusi porsi terakhir' : 'Laporan sekolah terakhir'}</p>
            </div>
          </header>
